(() => {
  const applyCopy = () => {
    ...
    if (section) {
      ...
      const cardContent = [
        { title: 'Accuracy & quality', risk: 'Generative AI can produce inaccurate, incomplete or convincing-looking output.', lylo: 'Lylo is built to ground answers in the firm’s own documents and show the source material for review.' },
        { title: 'Confidentiality & security', risk: 'Client information and firm systems need careful protection when AI tools are used.', lylo: 'Lylo’s private legal AI is designed so sensitive case work can remain inside the firm’s environment.' },
        { title: 'Human oversight', risk: 'AI output still needs proper human review before it is relied on in legal work.', lylo: 'Lylo keeps the solicitor in control, with checking, editing and review built into the workflow.' },
        { title: 'Client consent & transparency', risk: 'Firms should consider when clients need to be informed or consent may be required for particular AI uses.', lylo: 'Lylo is designed as a controlled workflow so firms can decide where AI is used and how it fits their process.' },
        { title: 'Professional duties', risk: 'Using AI does not remove the professional duties that apply to solicitors and firms.', lylo: 'Lylo supports <u>your</u> informed decisions.' }
      ];
      ...
      if (line) line.innerHTML = 'UK GDPR works alongside these professional responsibilities: data protection by design, data minimisation and security still apply.<span class="reg-built">Lylo is designed with these responsibilities in mind from the beginning.</span>';
      if (sources) sources.innerHTML = '<a href="https://lawscot.org.uk/media/pl1lnu5n/ai-guide.pdf" target="_blank" rel="noopener">Law Society of Scotland: Guide to Generative AI</a> &nbsp;·&nbsp; <a href="https://ico.org.uk/for-organisations/uk-gdpr-guidance-and-resources/accountability-and-governance/guide-to-accountability-and-governance/data-protection-by-design-and-by-default/" target="_blank" rel="noopener">ICO: data protection by design</a> &nbsp;·&nbsp; <a href="https://media.sra.org.uk/solicitors/guidance/misuse-ai/" target="_blank" rel="noopener">England & Wales: SRA AI warning</a>';
    }

    const privacyMain = document.querySelector('.privacy-main');
    if (privacyMain) privacyMain.textContent = 'What if your firm could use the benefits of AI without sending sensitive client data to a public AI service? With Lylo, case files stay on the firm’s own systems.';
